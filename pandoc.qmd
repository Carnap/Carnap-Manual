...
The `points` and `late-credit` options determine how many points to award a student 
who has submitted a correct solution (before and after the due date, respectively). 
*Only integer values are allowed.* The default for `points` is 5. The default for 
`late-credit` is 2, unless you've set a custom value for `points`, in which case 
the default is half that, rounded down to the nearest integer. Points automatically 
update if you change the due date (by assigning the same problem again with a 
different due date, or if you give the student a per-assignment alternate due date).

The `submission` attribute only takes `none` as an allowed value, i.e., 
`submission="none"` turns off the "Submit" button on an exercise.
...
allow for different options. Most of them support the options `exam` and `nocheck`. 
The `exam` option allows submission of incorrect exercises, and the `nocheck`
option disables the "Check" button on an exercise. 

Some exercise types allow additional attributes.
...
1. [Syntax Checking](./syntax-check.html)
2. [Translation](./translation.html)
3. [Truth Tables](./truth-tables.html)
4. [Derivations](./derivations.html)
5. [Model Checking](./modelchecker.html)
6. [Qualitative Problems](./qualitative.html)
7. [Sequent Calculus Problems](./sequent-calculus.html)
8. [Gentzen-Prawitz Natural Deduction Problems](./gentzen-ND.html)
